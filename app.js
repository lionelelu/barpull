...
// ===================================================================
// ===================================================================
// DB SCHEMA — single source of truth for data structure
// Used by: loadDB, onboarding, Supabase sync (Phase 1)
// ===================================================================
var DB_DEFAULTS = {
  settings: {
    dark: false,
    ...
    timerSound: 'beep',
    colorTheme: 'blue',
    startWeight: null,    // set in onboarding
    startReps: null,      // set in onboarding
    weightUnit: 'lb',     // 'lb' or 'kg'
    customDays: null      // JSON string of day map, or null for defaults
  },
  pullupLog: [],       // [{sem, pu, banda, diff}]
  weights: [],         // [{date, weight}]
  sessions: [],        // [{date, day, dayName, exercises, duration, seriesDone, feeling, notes, calories}]
  records: {},         // {exerciseName: {value, date}}
  totalTrainingTime: 0,
  streak: 0,
  lastTrainingDate: null,
  achievements: {}     // {badgeId: {date}}
};

// Stale keys to clean from settings on load (removed features)
var STALE_SETTINGS = ['ghUsername','ghRepo','ghToken','ghFileSha','ghLastSync','gdriveToken','gdriveClientId'];

var DB = JSON.parse(JSON.stringify(DB_DEFAULTS));
...
var SUPABASE_URL = 'https://didwiopvrroollfpsoso.supabase.co';
var SUPABASE_ANON_KEY = 'sb_publishable_1CeLW1J47jeR_KHnE4iEEA_z8yCT_tL';

var _sb = null;          // Supabase client instance
var _sbUser = null;      // Current auth user
var _sbReady = false;    // Client initialized
var _syncTimeout = null; // Debounce timer for cloud push
var _syncQueue = [];     // Offline queue
var _isSyncing = false;  // Prevent concurrent syncs
var _cloudVersion = 0;   // Optimistic concurrency version
...
function initSupabase() {
  ...
  try {
    ...
    _syncQueue = LS.getJSON('bp_sync_queue', []);
...
function loadDB() {
  try {
    var dbKey = getDBKey(getCurrentProfileId());
    var saved = localStorage.getItem(dbKey);
    if (saved) {
      var parsed = JSON.parse(saved);
      Object.assign(DB, parsed);
    }
    // Ensure all expected fields exist (schema migration safety)
    ...
    if (typeof DB.totalTrainingTime !== 'number') DB.totalTrainingTime = 0;
    if (typeof DB.streak !== 'number') DB.streak = 0;
    // Ensure settings object with all defaults
    if (!DB.settings || typeof DB.settings !== 'object') DB.settings = {};
    var sd = DB_DEFAULTS.settings;
    for (var k in sd) {
      if (DB.settings[k] === undefined) DB.settings[k] = sd[k];
    }
    // Clean stale settings from removed features
    STALE_SETTINGS.forEach(function(s) { delete DB.settings[s]; });
...
function saveDB() {
  ...
  // Cloud sync: debounced push if logged in
  if (isLoggedIn()) {
    if (navigator.onLine) {
      debouncedCloudPush();
    } else {
...
var NAV_MAP = {
  ...
  guia:     { pages: ['programa','bandas'], labels: ['Rutina','Bandas'] },
...
function goMain(mainId, btn) {
  // Block navigation if in guest mode (no profile)
  var _profiles = getProfiles();
  var _cur = _profiles.find(function(p) { return p.id === getCurrentProfileId(); });
  if (!_cur || !_cur.onboarded) {
    showGuestDashboard();
    return;
  }
  CURRENT_MAIN = mainId;
...
function go(id) {
  ...
  if (id === 'hoy') {
    renderHoyDate(); renderWeeklyChallenge();
    // Restore training UI if session was active
    if (HOY_STATE.day && HOY_STATE.seriesDone > 0) {
      var tp = document.getElementById('hoy-training');
      if (tp) tp.style.display = 'block';
      var sp = document.getElementById('hoy-start-panel');
      if (sp) sp.style.display = 'none';
    }
  }
  if (id === 'bandas' || id === 'programa') { renderDynamicUI(); }
  if (id === 'programa') { setTimeout(initProgramaExerciseLinks, 100); }
}
...
function selectHoyDay(dayNum, btn) {
  ...
  // If same day and already training or warmed up, don't reset
  if (HOY_STATE.day === dayNum && HOY_STATE.seriesDone > 0) return;
  if (HOY_STATE.day === dayNum && HOY_STATE.warmupDone) return;
...
// =====================================================================
// ADAPTIVE ROUTINE — adjusts based on user level
// =====================================================================
function getAdaptedDays() {
  var reps = getStartReps();
  var latest = DB.pullupLog && DB.pullupLog.length > 0 ? DB.pullupLog[DB.pullupLog.length-1].pu : reps;
  var level = Math.max(reps, latest);
  
  // Clone base DAYS
  var adapted = JSON.parse(JSON.stringify(DAYS));
  
  if (level <= 2) {
    // Beginner: fewer sets, more band work, more negatives
    adapted[1].exercises[0].sets = 3; // Wide pull-up: 3 instead of 4
    adapted[1].exercises[1].sets = 2; // Neutral: 2 instead of 3
    adapted[1].exercises[2].sets = 2; // Chin-up: 2 instead of 3
...
function buildTrainingList(dayNum) {
  var dayData = getAdaptedDays()[dayNum];
...
function getInstructionsHTML(exName) {
  ...
  if (!data || !data.steps || data.steps.length === 0) {
    return '<div style="font-size:13px;color:var(--ink3);padding:8px 0">No hay instrucciones disponibles para este ejercicio.</div>';
  }
  var html = '';
  if (data.vid) {
  ...
  html += '</ol>';
  // If no video, offer search
  if (!data.vid) {
    var searchQ = encodeURIComponent(key + ' ejercicio tutorial');
    html += '<div style="margin-top:8px"><a href="https://www.youtube.com/results?search_query=' + searchQ + '" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:6px;background:var(--s2);border:1px solid var(--border);border-radius:6px;padding:6px 12px;font-size:12px;color:var(--ink2);text-decoration:none;font-family:Inter,sans-serif">🔍 Buscar video en YouTube</a></div>';
  }
  if (data.tip) {
...
function completeAllExercises() {
  ...
  document.getElementById('sum-ex').textContent = getAdaptedDays()[HOY_STATE.day].exercises.length;
  // Calorie estimation
...
// =====================================================================
// INLINE EXERCISE DETAILS (in Programa tab)
// =====================================================================
function initProgramaExerciseLinks() {
  // Make exercise names in programa tables clickable
  var exNames = document.querySelectorAll('#programa .en');
  exNames.forEach(function(el) {
    var name = el.textContent.trim().split('+')[0].split('--')[0].trim();
    // Remove any existing decoration that conflicts
    var cleanName = name.replace(/ *\u2014.*/, '').replace(/ *\+ .*/, '').trim();
    el.style.cursor = 'pointer';
    el.style.textDecoration = 'underline';
    el.style.textDecorationColor = 'var(--border2)';
    el.style.textUnderlineOffset = '2px';
    el.onclick = function(e) {
      e.stopPropagation();
      toggleInlineExercise(el, cleanName);
    };
  });
}

function toggleInlineExercise(el, exName) {
  // Find the parent <tr> and add detail row below it
  var tr = el.closest('tr');
  if (!tr) return;
  // Check if already expanded
  var nextRow = tr.nextElementSibling;
  if (nextRow && nextRow.classList.contains('inline-detail-row')) {
    nextRow.remove();
    return;
  }
  // Close any other open details
  document.querySelectorAll('.inline-detail-row').forEach(function(d) { d.remove(); });
  // Get exercise data — try exact name first, then aliases
  var aliases = {
    'Wide pull-up': 'Wide pull-up',
    'Narrow pull-up': 'Neutral pull-up',
    'Curl biceps': 'Face pull + curl con banda amarilla',
    'Curl b\u00edceps': 'Face pull + curl con banda amarilla',
    'Sit-up con pies en barra': 'Reverse crunch',
  };
  var html = getInstructionsHTML(exName);
  if (!html || html.includes('Ver el tutorial')) {
    for (var key in aliases) {
      if (exName.toLowerCase().includes(key.toLowerCase())) {
        html = getInstructionsHTML(aliases[key]);
        if (html && !html.includes('Ver el tutorial')) break;
      }
    }
  }
  if (!html) return;
  // Create a full-width row below
  var detailRow = document.createElement('tr');
  detailRow.className = 'inline-detail-row';
  var td = document.createElement('td');
  td.colSpan = 3;
  td.style.cssText = 'padding:0;border-bottom:1px solid var(--border)';
  td.innerHTML = '<div class="inline-ex-detail">' + html + '</div>';
...
function renderPeso() {
  ...
  var todayEntry = DB.weights.find(function(w) { return w.date === todayStr; });
  var todayEl = document.getElementById('peso-hoy-val');
  if (todayEl) todayEl.textContent = todayEntry ? todayEntry.weight : '—';
  // Update input + save button based on state
  var inp = document.getElementById('peso-input');
  var saveBtn = document.getElementById('peso-save-btn');
  if (todayEntry) {
    if (inp) { inp.value = todayEntry.weight; inp.disabled = true; inp.style.opacity = '0.6'; }
    if (saveBtn) { saveBtn.textContent = '✓ Guardado'; saveBtn.classList.remove('btn-blue'); saveBtn.classList.add('btn-ghost'); saveBtn.style.pointerEvents = 'none'; }
  } else {
    if (inp) { inp.value = ''; inp.disabled = false; inp.style.opacity = '1'; }
    if (saveBtn) { saveBtn.textContent = 'Guardar ✓'; saveBtn.classList.add('btn-blue'); saveBtn.classList.remove('btn-ghost'); saveBtn.style.pointerEvents = 'auto'; }
  }
  // Inicio
...
function savePeso() {
  ...
  if (!rawVal || isNaN(val) || val < 30 || val > 500) {
  ...
  if (existing >= 0) {
    showToast('Ya registraste peso hoy. Usa el ✏️ en el historial para editarlo.', '⚠️', 3500);
    return;
  }
  DB.weights.unshift({ date: todayStr, weight: val });
  saveDB();
  renderPeso();
  renderQuickWeight();
  checkAchievements();
  showToast('Peso guardado: ' + val + ' ' + getWeightUnit(), '⚖️');
}
...
function renderPesoLog() {
  ...
  DB.weights.slice(0,30).forEach(function(w, i) {
    ...
    html += '<div class="weight-log-row"><div class="weight-log-date">'+w.date+'</div><div style="display:flex;align-items:center;gap:8px"><div class="weight-log-val" id="wlog-val-'+i+'">'+w.weight+' '+_wu+'</div>'+diffHtml;
    // Edit pencil button
    html += '<button type="button" onclick="editWeight('+i+',\''+w.date+'\')" style="background:none;border:none;color:var(--ink3);cursor:pointer;font-size:12px;padding:2px 4px;opacity:.6" title="Editar">✏️</button>';
    // Delete X button
    html += '<button type="button" onclick="deleteWeight(\''+w.date+'\',this)" style="background:none;border:none;color:var(--ink3);cursor:pointer;font-size:12px;padding:2px 4px;opacity:.4" title="Eliminar">✕</button>';
    html += '</div></div>';
  });
  el.innerHTML = html;
}

function editWeight(idx, dateStr) {
  var row = document.getElementById('wlog-val-' + idx);
  if (!row) return;
  var entry = DB.weights.find(function(w) { return w.date === dateStr; });
  if (!entry) return;
  var unit = getWeightUnit();
  // Replace value display with inline edit
  row.innerHTML = '<input type="number" id="wlog-edit-' + idx + '" value="' + entry.weight + '" step="0.1" min="30" max="500" style="width:70px;font-size:13px;font-weight:600;padding:2px 6px;border:1px solid var(--blue);border-radius:var(--rs);text-align:center;font-family:JetBrains Mono,monospace"> ' + unit + ' <button type="button" onclick="saveWeightEdit(' + idx + ',\'' + dateStr + '\')" style="background:var(--blue);color:#fff;border:none;border-radius:var(--rs);padding:2px 8px;font-size:11px;cursor:pointer;font-family:Inter,sans-serif">✓</button> <button type="button" onclick="renderPesoLog()" style="background:none;border:none;color:var(--ink3);font-size:11px;cursor:pointer">✕</button>';
  var inp = document.getElementById('wlog-edit-' + idx);
  if (inp) { inp.focus(); inp.select(); }
}

function saveWeightEdit(idx, dateStr) {
  var inp = document.getElementById('wlog-edit-' + idx);
  if (!inp) return;
  var val = parseFloat(inp.value);
  if (isNaN(val) || val < 30 || val > 500) { showToast('Peso inválido', '⚠️'); return; }
  val = Math.round(val * 10) / 10;
  var entry = DB.weights.find(function(w) { return w.date === dateStr; });
  if (entry) {
    entry.weight = val;
    ...
    renderPeso();
    renderQuickWeight();
    showToast('Peso actualizado: ' + val + ' ' + getWeightUnit(), '✏️');
  }
}
...
function generateShareCard() {
  ...
  ctx.font = '10px JetBrains Mono, monospace';
  ctx.textAlign = 'right';
  ctx.fillText('barpull · ' + localDateStr(), W - 20, H - 15);
  ctx.textAlign = 'left';
  return canvas;
}
...
function shareProgress() {
  ...
  canvas.toBlob(function(blob) {
    ...
    if (navigator.share && navigator.canShare) {
      var file = new File([blob], 'barpull-progreso.png', { type: 'image/png' });
      var shareData = { files: [file], title: 'Mi progreso — BarPull' };
...
function downloadShareImage(blob) {
  ...
  a.href = url;
  a.download = 'barpull-progreso-' + localDateStr() + '.png';
  a.click();
...
function renderDynamicUI() {
  ...
  // Quick weight widget (dynamic render)
  renderQuickWeight();
}
...
function renderHeatmap() {
  ...
  while (d <= today) {
    ...
    for (var dow = 0; dow < 7; dow++) {
      ...
      html += '<div class="hm-cell" style="background:' + bg + ';border:' + border + '" title="' + title + '"></div>';
  ...
  // Empty state
  if (totalDays === 0) {
    html += '<div style="text-align:center;padding:8px 0;font-size:13px;color:var(--ink3)">Completa tu primer entrenamiento para ver tu actividad aqui.</div>';
  }
  el.innerHTML = html;
}
...
function setupNotifications() {
  ...
  Notification.requestPermission().then(function(perm) {
    ...
    if (perm === 'granted') {
      ...
      new Notification('BarPull', { body: '¡Notificaciones activadas! Te avisaremos cuando sea hora de entrenar.', icon: '' });
    } else {
...
async function confirmReset() {
  var ok = await modalDanger('Borrar perfil', 'Se borrarán TODOS los datos de este perfil y se eliminará. Esta acción no se puede deshacer.');
  if (ok) {
    ...
    var profiles = getProfiles().filter(function(p) { return p.id !== profileId; });
    saveProfiles(profiles);
    // Switch to another profile or guest mode
    if (profiles.length > 0) {
      LS.set(CURRENT_PROFILE_KEY, profiles[0].id);
    } else {
...
async function confirmFullReset() {
  var ok1 = await modalDanger('Reset total', 'Esto borra TODOS los perfiles, datos y configuracion — absolutamente todo vuelve a cero.');
  if (!ok1) return;
  ...
  for (var i = 0; i < localStorage.length; i++) {
    ...
    if (key && (key.startsWith('barpull') || key.startsWith('ig_'))) {
      keys.push(key);
    }
  }
...
function exportData() {
  ...
  var a = document.createElement('a');
  a.href = url;
  a.download = 'barpull_datos_' + localDateStr() + '.json';
  a.click();
  URL.revokeObjectURL(url);
}
...
function checkBackupReminder() {
  var lastBackup = LS.get('ig_last_backup', '0');
  var daysSince = Math.floor((Date.now() - parseInt(lastBackup)) / (1000*60*60*24));
  var sessions = DB.sessions ? DB.sessions.length : 0;
  // Remind every 7 days if they have 3+ sessions and never backed up
  if (sessions >= 3 && daysSince >= 7) {
    setTimeout(function() {
      showModal({
        title: 'Protege tus datos',
        message: 'Llevas ' + sessions + ' sesiones. Te recomendamos hacer un respaldo por si se borran los datos del navegador.',
        icon: '💾',
        type: 'confirm',
        confirmText: 'Hacer backup ahora',
        cancelText: 'Recordarme luego'
      }).then(function(ok) {
        if (ok) {
          shareBackupFile();
          LS.set('ig_last_backup', String(Date.now()));
        }
      });
    }, 5000);
  }
...
window.addEventListener('DOMContentLoaded', function() {
  ...
  var vfEl = document.getElementById('app-version-footer');
  if (vfEl) vfEl.textContent = 'v' + APP_VERSION;
  // Init profiles
  profilesInit();
  // Check if guest mode
  checkGuestMode();
  ...
  // Init Supabase (cloud sync)
  initSupabase();
  // Render cloud account UI in settings
  renderCloudAccountUI();
  // Check backup reminder
  checkBackupReminder();
  // PWA Manifest injection
  (function() {
    var manifest = {"name":"BarPull","short_name":"BarPull","description":"Tu rutina de dominadas","start_url":"./","display":"standalone","background_color":"#F6F5F1","theme_color":"#2563EB","orientation":"portrait-primary"};
    try {
...
// ===================================================================
// EASY SYNC (QR Code + Share)
// ===================================================================

function compressData() {
  // Create a compact version of DB for sharing
  var data = JSON.parse(JSON.stringify(DB));
  // Remove local-only keys before sharing
...
function generateSyncQR() {
  ...
  if (size > 2500) {
    // Too big for QR — offer file share instead
    showModal({
      title: 'Datos muy grandes para QR',
      message: 'Tienes ' + Math.round(size/1024) + 'KB de datos. Usa "Compartir archivo" para enviar por WhatsApp o mensaje.',
      icon: '📦',
      type: 'alert',
  ...
  // Show QR modal
  var modal = document.getElementById('qr-sync-modal');
  if (!modal) return;
  modal.classList.add('show');
  ...
  document.getElementById('qr-data-size').textContent = Math.round(size/1024) + 'KB';
}

function generateQRCode(canvas, data) {
  // Minimal QR Code generator (works for our data sizes)
  // Using a simple approach: encode as data URL for scanning
  var encoded = btoa(unescape(encodeURIComponent(data)));
  var url = 'BPULL:' + encoded;
  
  // Draw QR using a simple matrix
  var ctx = canvas.getContext('2d');
  var size = 280;
  ...
  // Since we can not include a full QR library, we draw a placeholder
  // and provide the data as copyable text instead
  ctx.fillStyle = '#1a1917';
  ctx.font = 'bold 16px Inter, sans-serif';
  ctx.textAlign = 'center';
  ctx.fillText('Codigo de sincronizacion', size/2, 30);
  
  // Draw a visual code pattern (decorative)
  var blockSize = 8;
  var margin = 40;
  var gridW = Math.floor((size - margin*2) / blockSize);
  var hash = 0;
  for (var i = 0; i < encoded.length; i++) {
    hash = ((hash << 5) - hash) + encoded.charCodeAt(i);
    hash |= 0;
  }
  for (var y = 0; y < gridW && y < 24; y++) {
    for (var x = 0; x < gridW && x < 24; x++) {
      var val = (encoded.charCodeAt((y * gridW + x) % encoded.length) + hash + x * y) & 1;
      if (val) {
        ctx.fillRect(margin + x * blockSize, 44 + y * blockSize, blockSize - 1, blockSize - 1);
      }
    }
  }
  ...
  ctx.fillText('Usa "Copiar codigo" abajo', size/2, size - 10);
}

function copySyncCode() {
  var data = compressData();
  var code = 'BPULL:' + btoa(unescape(encodeURIComponent(data)));
  
  if (navigator.clipboard && navigator.clipboard.writeText) {
    navigator.clipboard.writeText(code).then(function() {
      showToast('Codigo copiado. Pegalo en el otro dispositivo.', '📋', 4000);
    }).catch(function() {
      fallbackCopy(code);
    });
  } else {
...
function fallbackCopy(text) {
  ...
  ta.remove();
  showToast('Codigo copiado', '📋', 3000);
}

function shareBackupFile() {
  var data = compressData();
  var blob = new Blob([data], { type: 'application/json' });
  var fileName = 'barpull-backup-' + localDateStr() + '.json';
  
  if (navigator.share && navigator.canShare) {
    var file = new File([blob], fileName, { type: 'application/json' });
    var shareData = { files: [file], title: 'BarPull - Backup' };
    if (navigator.canShare(shareData)) {
...
function downloadBackupFile(blob, fileName) {
  ...
  URL.revokeObjectURL(url);
  showToast('Archivo descargado. Envialo por WhatsApp o email.', '📤', 4000);
}

function showImportSync() {
  var modal = document.getElementById('import-sync-modal');
  if (modal) modal.classList.add('show');
}

function hideImportSync() {
  var modal = document.getElementById('import-sync-modal');
  if (modal) modal.classList.remove('show');
}
...
async function importSyncCode() {
  ...
  if (!code) {
    showToast('Pega el codigo de sincronizacion', '⚠️');
    return;
  }
  
  // Handle BPULL: prefix
  if (code.startsWith('BPULL:')) code = code.substring(6);
  ...
  try {
    ...
    var ok = await modalConfirm(
      'Importar datos',
      'Se encontraron ' + (imported.sessions ? imported.sessions.length : 0) + ' sesiones y ' + (imported.weights ? imported.weights.length : 0) + ' registros de peso. ¿Reemplazar tus datos actuales?',
      '📥'
    );
    ...
    if (ok) {
      ...
      var localSettings = {
        dark: DB.settings.dark,
        timerSound: DB.settings.timerSound,
        colorTheme: DB.settings.colorTheme
      };
      Object.assign(DB, imported);
      if (!DB.settings) DB.settings = {};
      Object.assign(DB.settings, localSettings);
      saveDBLocal();
      renderLog(); renderTotalStats(); renderHoyDate(); renderDynamicUI();
      hideImportSync();
      showToast('Datos importados correctamente', '✅', 3000);
    }
  } catch(e) {
    showToast('Codigo invalido. Verifica que lo copiaste completo.', '❌', 4000);
  }
}
...
function renderAuthUI() {
  ...
  if (_authMode === 'login') {
    ...
    html += '<div style="font-size:20px;font-weight:700;color:var(--ink)">Iniciar sesion</div>';
    html += '<div style="font-size:13px;color:var(--ink3);margin-top:4px">Accede a tus datos desde cualquier dispositivo</div></div>';
    html += '<input type="email" id="auth-email" class="sync-input" placeholder="Email" autocomplete="email">';
    html += '<input type="password" id="auth-pass" class="sync-input" placeholder="Contrasena" autocomplete="current-password">';
    html += '<div id="auth-error" style="display:none;font-size:12px;color:var(--red);margin:4px 0 8px;text-align:center"></div>';
    html += '<button type="button" class="btn btn-blue" onclick="authSignIn()" style="width:100%;justify-content:center;margin-top:8px" id="auth-submit-btn">🔑 Entrar</button>';
    html += '<div style="text-align:center;margin-top:12px">';
    html += '<button type="button" style="background:none;border:none;color:var(--blue);font-size:13px;cursor:pointer;font-family:Inter,sans-serif;text-decoration:underline" onclick="switchAuthMode(\'reset\')">Olvide mi contrasena</button>';
    html += '</div>';
    html += '<div style="text-align:center;margin-top:8px;font-size:13px;color:var(--ink3)">No tienes cuenta? ';
    html += '<button type="button" style="background:none;border:none;color:var(--blue);font-size:13px;cursor:pointer;font-family:Inter,sans-serif;font-weight:600" onclick="switchAuthMode(\'register\')">Crear una</button></div>';

  } else if (_authMode === 'register') {
    html += '<div style="text-align:center;margin-bottom:20px">';
    html += '<div style="font-size:48px;margin-bottom:8px">📧</div>';
    html += '<div style="font-size:20px;font-weight:700;color:var(--ink)">Crea tu cuenta</div>';
    html += '<div style="font-size:13px;color:var(--ink3);margin-top:4px">Sincroniza entre dispositivos, nunca pierdas tus datos</div></div>';
    html += '<input type="email" id="auth-email" class="sync-input" placeholder="Email" autocomplete="email">';
    html += '<input type="password" id="auth-pass" class="sync-input" placeholder="Contrasena (min 6 caracteres)" autocomplete="new-password">';
    html += '<input type="password" id="auth-pass2" class="sync-input" placeholder="Confirmar contrasena" autocomplete="new-password">';
    html += '<div id="auth-error" style="display:none;font-size:12px;color:var(--red);margin:4px 0 8px;text-align:center"></div>';
    html += '<button type="button" class="btn btn-blue" onclick="authSignUp()" style="width:100%;justify-content:center;margin-top:8px" id="auth-submit-btn">📧 Crear cuenta</button>';
    html += '<div style="text-align:center;margin-top:12px;font-size:13px;color:var(--ink3)">Ya tienes cuenta? ';
    html += '<button type="button" style="background:none;border:none;color:var(--blue);font-size:13px;cursor:pointer;font-family:Inter,sans-serif;font-weight:600" onclick="switchAuthMode(\'login\')">Iniciar sesion</button></div>';

  } else if (_authMode === 'reset') {
    html += '<div style="text-align:center;margin-bottom:20px">';
    html += '<div style="font-size:48px;margin-bottom:8px">🔓</div>';
    html += '<div style="font-size:20px;font-weight:700;color:var(--ink)">Recuperar contrasena</div>';
    html += '<div style="font-size:13px;color:var(--ink3);margin-top:4px">Te enviaremos un enlace para crear una nueva</div></div>';
    html += '<input type="email" id="auth-email" class="sync-input" placeholder="Email de tu cuenta" autocomplete="email">';
    html += '<div id="auth-error" style="display:none;font-size:12px;color:var(--red);margin:4px 0 8px;text-align:center"></div>';
    html += '<button type="button" class="btn btn-blue" onclick="authResetPassword()" style="width:100%;justify-content:center;margin-top:8px" id="auth-submit-btn">📤 Enviar enlace</button>';
    html += '<div style="text-align:center;margin-top:12px">';
    html += '<button type="button" style="background:none;border:none;color:var(--blue);font-size:13px;cursor:pointer;font-family:Inter,sans-serif" onclick="switchAuthMode(\'login\')">← Volver a iniciar sesion</button></div>';
  }

  // Cancel button (always)
  html += '<button type="button" class="btn btn-ghost btn-sm" onclick="hideAuthOverlay()" style="width:100%;justify-content:center;margin-top:10px">Cancelar</button>';
...
async function authSignUp() {
  if (!_sbReady) { showAuthError('Servicio no disponible. Intenta mas tarde.'); return; }
  hideAuthError();
  var email = (document.getElementById('auth-email').value || '').trim();
  var pass = document.getElementById('auth-pass').value || '';
  var pass2 = document.getElementById('auth-pass2').value || '';

  if (!email || !email.includes('@')) { showAuthError('Ingresa un email valido'); return; }
  if (pass.length < 6) { showAuthError('La contrasena debe tener al menos 6 caracteres'); return; }
  if (pass !== pass2) { showAuthError('Las contrasenas no coinciden'); return; }

  setAuthLoading(true);
  try {
    var profiles = getProfiles();
    var currentProfile = profiles.find(function(p) { return p.id === getCurrentProfileId(); });
    var displayName = (currentProfile && currentProfile.name) ? currentProfile.name : '';
    ...
    if (result.data.user && !result.data.session) {
      ...
      showModal({
        ...
        message: 'Te enviamos un enlace de confirmacion a <strong>' + sanitize(email) + '</strong>. Abrelo para activar tu cuenta.',
        icon: '📧',
        type: 'alert',
        confirmText: 'Entendido'
      });
      ...
      hideAuthOverlay();
      showToast('Cuenta creada correctamente', '✅', 3000);
    }
  } catch(e) {
    showAuthError('Error inesperado. Intenta de nuevo.');
...
async function authSignIn() {
  ...
  if (!email || !email.includes('@')) { showAuthError('Ingresa un email valido'); return; }
  if (!pass) { showAuthError('Ingresa tu contrasena'); return; }
  ...
  try {
    ...
    hideAuthOverlay();
    showToast('Sesion iniciada', '✅', 3000);
    // Data pull happens in handleAuthStateChange
  } catch(e) {
    showAuthError('Error inesperado. Intenta de nuevo.');
...
async function authSignOut() {
  ...
  var confirmed = await showModal({
    title: 'Cerrar sesion',
    message: 'Tu datos locales se mantienen, pero dejaran de sincronizarse con la nube.',
    icon: '👋',
    type: 'confirm',
  ...
  try {
    ...
    _cloudVersion = 0;
    _syncQueue = [];
    LS.remove('bp_sync_queue');
    renderCloudAccountUI();
    showToast('Sesion cerrada', '👋', 3000);
  } catch(e) {
...
async function authResetPassword() {
  if (!_sbReady) { showAuthError('Servicio no disponible.'); return; }
  hideAuthError();
  var email = (document.getElementById('auth-email').value || '').trim();
  if (!email || !email.includes('@')) { showAuthError('Ingresa tu email'); return; }
  ...
  try {
    ...
    showModal({
      ...
      message: 'Revisa tu email <strong>' + sanitize(email) + '</strong> para cambiar tu contrasena.',
      icon: '📧',
      type: 'alert',
      confirmText: 'Entendido'
    });
  } catch(e) {
    showAuthError('Error inesperado.');
  }
  setAuthLoading(false);
}
...
function translateAuthError(msg) {
  ...
  if (m.includes('invalid login')) return 'Email o contrasena incorrectos';
  if (m.includes('email not confirmed')) return 'Confirma tu email primero (revisa tu bandeja)';
  if (m.includes('user already registered')) return 'Ya existe una cuenta con ese email';
  if (m.includes('password') && m.includes('least')) return 'La contrasena debe tener al menos 6 caracteres';
  if (m.includes('rate limit') || m.includes('too many')) return 'Demasiados intentos. Espera un momento.';
  if (m.includes('network')) return 'Error de conexion. Verifica tu internet.';
  return msg;
}
...
async function handleAuthStateChange(event, session) {
  if (event === 'SIGNED_IN' || event === 'TOKEN_REFRESHED') {
    ...
    // User clicked password reset link — show password change UI
    showToast('Ingresa tu nueva contrasena', '🔑', 5000);
    showAuthOverlay('login');
  }
  // Update offline banner text based on auth state
  updateOfflineBannerText();
}

// ===================================================================
// CLOUD SYNC — PUSH / PULL / MERGE
// ===================================================================
function debouncedCloudPush() {
  if (_syncTimeout) clearTimeout(_syncTimeout);
  _syncTimeout = setTimeout(function() { cloudPush(); }, 2000);
}

async function cloudPush() {
  if (!isLoggedIn() || _isSyncing || !navigator.onLine) return;
  _isSyncing = true;
  updateSyncIndicator('syncing');
  try {
    var dataToSync = JSON.parse(JSON.stringify(DB));
    // Strip local-only keys before pushing
    delete dataToSync._localOnly;

    var result = await _sb.from('user_data')
      .upsert({
        id: _sbUser.id,
        profile_data: dataToSync,
        version: _cloudVersion + 1
      }, { onConflict: 'id' })
      .select('version')
      .single();

    if (result.error) {
      console.error('cloudPush error:', result.error);
      updateSyncIndicator('error');
    } else {
      _cloudVersion = result.data.version;
      updateSyncIndicator('synced');
      // Update profile in user_profiles table
      var profiles = getProfiles();
      var current = profiles.find(function(p) { return p.id === getCurrentProfileId(); });
      if (current) {
        await _sb.from('user_profiles').upsert({
          id: _sbUser.id,
          display_name: current.name || '',
          emoji: current.emoji || '',
          color_theme: (DB.settings && DB.settings.colorTheme) || 'blue',
          onboarded: !!current.onboarded
        }, { onConflict: 'id' });
      }
      // Log sync event
      await _sb.from('sync_log').insert({
        user_id: _sbUser.id,
        action: 'full_push',
    ...
    console.error('cloudPush exception:', e);
    updateSyncIndicator('error');
  }
  _isSyncing = false;
}
...
async function cloudPull() {
  ...
  try {
    ...
    if (result.error) {
      // No data row yet (new account) — that's OK
      if (result.error.code === 'PGRST116') {
        updateSyncIndicator('synced');
        return;
      }
      console.error('cloudPull error:', result.error);
      updateSyncIndicator('error');
      return;
    }

    var cloudData = result.data.profile_data;
    _cloudVersion = result.data.version;

    // If cloud is empty or has no sessions, keep local data
    if (!cloudData || (typeof cloudData === 'object' && Object.keys(cloudData).length === 0)) {
      updateSyncIndicator('synced');
      return;
    }

    // Cloud has data — check if we should merge or overwrite
    var localHasData = DB.sessions && DB.sessions.length > 0;
    var cloudHasData = cloudData.sessions && cloudData.sessions.length > 0;

    if (cloudHasData && !localHasData) {
      // Cloud has data, local is empty → use cloud
      applyCloudData(cloudData);
      showToast('Datos cargados desde la nube', '☁️', 3000);
    } else if (cloudHasData && localHasData) {
      // Both have data — use cloud as source of truth (last synced wins)
      applyCloudData(cloudData);
      showToast('Datos sincronizados desde la nube', '☁️', 3000);
    }
    // If cloud is empty but local has data → push will happen via migration check
    ...
    console.error('cloudPull exception:', e);
    updateSyncIndicator('error');
  }
}

function applyCloudData(cloudData) {
  // Merge cloud data into DB, preserving defaults for missing fields
  var merged = JSON.parse(JSON.stringify(DB_DEFAULTS));
  Object.assign(merged, cloudData);
  // Ensure settings defaults
  if (!merged.settings || typeof merged.settings !== 'object') merged.settings = {};
  var sd = DB_DEFAULTS.settings;
  for (var k in sd) {
    if (merged.settings[k] === undefined) merged.settings[k] = sd[k];
  }
  STALE_SETTINGS.forEach(function(s) { delete merged.settings[s]; });
  // Apply to live DB
  ...
  try {
    ...
    renderLog();
    renderTotalStats();
    if (DB.settings.dark) document.documentElement.setAttribute('data-theme','dark');
    else document.documentElement.setAttribute('data-theme','light');
    if (DB.settings.colorTheme) applyColorTheme(DB.settings.colorTheme);
  } catch(e) { console.log('UI re-render after cloud pull:', e); }
}
...
async function manualSync() {
  ...
  if (!navigator.onLine) {
    showToast('Sin conexion — intenta cuando tengas internet', '📡', 3000);
    return;
  }
  showToast('Sincronizando...', '☁️', 2000);
  await cloudPush();
  showToast('Sincronizado', '✅', 2000);
}
...
function addToSyncQueue(action) {
  _syncQueue.push({ action: action, timestamp: Date.now() });
  // Keep only last 20 entries to avoid bloat
  if (_syncQueue.length > 20) _syncQueue = _syncQueue.slice(-20);
  LS.setJSON('bp_sync_queue', _syncQueue);
}

async function flushSyncQueue() {
  if (!isLoggedIn() || !navigator.onLine || _syncQueue.length === 0) return;
  // Clear queue first to avoid re-processing
  var pending = _syncQueue.slice();
  _syncQueue = [];
  LS.setJSON('bp_sync_queue', []);
  // Just do one push (all changes are in DB)
  var hasPush = pending.some(function(q) { return q.action === 'push'; });
  if (hasPush) {
    await cloudPush();
  }
}
...
// ===================================================================
// LOCAL DATA MIGRATION (local → cloud)
// ===================================================================
async function checkLocalDataMigration() {
  ...
  try {
    ...
    var cloudEmpty = !result.data || !result.data.profile_data ||
      (typeof result.data.profile_data === 'object' && Object.keys(result.data.profile_data).length === 0) ||
      (!result.data.profile_data.sessions || result.data.profile_data.sessions.length === 0);

    if (cloudEmpty) {
      // Cloud is empty, local has data → offer migration
      var confirmed = await showModal({
        title: 'Migrar datos a la nube',
        message: 'Tienes ' + DB.sessions.length + ' sesiones guardadas localmente. Quieres subirlas a tu cuenta para sincronizarlas entre dispositivos?',
        icon: '☁️',
        type: 'confirm',
        confirmText: 'Si, migrar',
        cancelText: 'Ahora no'
      });
      if (confirmed) {
        await cloudPush();
        showToast('Datos migrados a la nube', '✅', 3000);
      }
    }
  } catch(e) {
...
// ===================================================================
// SYNC INDICATOR + CLOUD ACCOUNT UI
// ===================================================================
function updateSyncIndicator(state) {
  // state: 'synced', 'syncing', 'error', 'offline'
  var dot = document.getElementById('cloud-sync-dot');
  var label = document.getElementById('cloud-sync-label');
  if (!dot || !label) return;
  dot.className = 'sync-dot';
  if (state === 'synced') { dot.classList.add('connected'); label.textContent = 'Sincronizado'; }
  else if (state === 'syncing') { dot.classList.add('syncing'); label.textContent = 'Sincronizando...'; }
  else if (state === 'error') { dot.classList.add('disconnected'); label.textContent = 'Error de sync'; }
  else if (state === 'offline') { dot.classList.add('disconnected'); label.textContent = 'Sin conexion'; }
}

function renderCloudAccountUI() {
  var el = document.getElementById('cloud-account-content');
  if (!el) return;
  var html = '';

  if (isLoggedIn()) {
    var email = _sbUser.email || '';
    html += '<div class="sync-status" style="margin-bottom:12px">';
    html += '<div class="sync-dot connected" id="cloud-sync-dot"></div>';
    html += '<div style="flex:1;min-width:0"><div style="font-size:13px;font-weight:600;color:var(--ink);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + sanitize(email) + '</div>';
    html += '<div style="font-size:11px;color:var(--ink3)" id="cloud-sync-label">Sincronizado</div></div>';
    html += '</div>';
    html += '<div style="display:flex;flex-direction:column;gap:8px">';
    html += '<button type="button" class="btn btn-blue btn-sm" onclick="manualSync()" style="justify-content:center">🔄 Sincronizar ahora</button>';
    html += '<button type="button" class="btn btn-ghost btn-sm" onclick="authSignOut()" style="justify-content:center">👋 Cerrar sesion</button>';
    html += '</div>';
    // Show pending queue if any
    if (_syncQueue.length > 0) {
      html += '<div style="font-size:11px;color:var(--amber);margin-top:8px">⏳ ' + _syncQueue.length + ' cambio(s) pendiente(s) de sincronizar</div>';
    }
  } else if (isSupabaseConfigured()) {
    html += '<p style="font-size:13px;color:var(--ink2);margin-bottom:14px">Crea una cuenta para sincronizar tus datos entre dispositivos.</p>';
    html += '<div style="display:flex;flex-direction:column;gap:8px">';
    html += '<button type="button" class="btn btn-blue btn-sm" onclick="showAuthOverlay(\'register\')" style="justify-content:center">📧 Crear cuenta gratis</button>';
    html += '<button type="button" class="btn btn-ghost btn-sm" onclick="showAuthOverlay(\'login\')" style="justify-content:center">🔑 Ya tengo cuenta</button>';
    html += '</div>';
  } else {
    html += '<p style="font-size:13px;color:var(--ink3)">La sincronizacion en la nube no esta configurada.</p>';
  }
...
function updateOfflineBannerText() {
  ...
  if (!banner) return;
  if (isLoggedIn()) {
    banner.textContent = '📡 Sin conexion — los cambios se sincronizaran al reconectar';
  } else {
    banner.textContent = '📡 Sin conexion — los datos se guardan localmente';
  }
}
...
function checkGuestMode() {
  if (window._skipGuestCheck) return false;
  var profiles = getProfiles();
  var currentId = getCurrentProfileId();
...
function showGuestDashboard() {
  var el = document.getElementById('guest-dashboard');
  if (el) el.style.display = 'block';
  var main = document.querySelector('.main');
  if (main) main.style.display = 'none';
  // Hide sub-nav and timer in guest mode
  var subNav = document.getElementById('sub-nav');
  if (subNav) subNav.style.display = 'none';
  var timerBar = document.getElementById('timerBar');
  if (timerBar) timerBar.style.display = 'none';
  // Dim nav buttons
  document.querySelectorAll('.nb').forEach(function(b) { b.style.opacity = '0.3'; b.style.pointerEvents = 'none'; });
}
...
function hideGuestDashboard() {
  ...
  // Re-enable nav
  document.querySelectorAll('.nb').forEach(function(b) { b.style.opacity = '1'; b.style.pointerEvents = 'auto'; });
}



function guestStartSetup() {
  var currentId = getCurrentProfileId();
  startOnboarding(currentId);
}
...
function renderOnboardingStep() {
  ...
  if (step === 1) {
    ...
    html += '<div style="font-size:20px;font-weight:700;color:var(--ink)">¿Cómo te llamas?</div>';
    html += '<div style="font-size:13px;color:var(--ink3);margin-top:4px">Este nombre aparece en tu perfil</div></div>';
    html += '<input type="text" id="ob-name" class="sync-input" placeholder="Tu nombre" maxlength="20" style="text-align:center;font-size:18px;font-weight:600"';
    if (ONBOARDING.data.name) html += ' value="' + sanitize(ONBOARDING.data.name) + '"';
    html += '>';

  } else if (step === 2) {
    // STEP 2: Emoji + Color
    html += '<div style="text-align:center;margin-bottom:20px"><div style="font-size:48px;margin-bottom:8px">🎨</div>';
    html += '<div style="font-size:20px;font-weight:700;color:var(--ink)">Personaliza tu perfil</div>';
    html += '<div style="font-size:13px;color:var(--ink3);margin-top:4px">Elige un avatar y color para ' + sanitize(ONBOARDING.data.name || '') + '</div></div>';
    // Emoji grid
    html += '<div style="font-size:12px;font-weight:600;color:var(--ink2);margin-bottom:6px">Emoji</div>';
    html += '<div class="profile-emoji-grid" id="ob-emoji-grid"></div>';
    // Color grid
    html += '<div style="font-size:12px;font-weight:600;color:var(--ink2);margin-bottom:6px;margin-top:14px">Color del tema</div>';
    html += '<div class="profile-color-grid" id="ob-color-grid"></div>';
    ...
    html += '<div style="font-size:13px;color:var(--ink3);margin-top:4px">Para calcular tu banda y estimar calorías</div></div>';
    ...
    html += '>';
    html += '<span style="font-size:18px;color:var(--ink3);font-weight:500">' + unit + '</span></div>';

  } else if (step === 4) {
    // STEP 4: Starting pull-ups
    html += '<div style="text-align:center;margin-bottom:20px"><div style="font-size:48px;margin-bottom:8px">💪</div>';
    ...
    html += '<div style="text-align:center;margin-bottom:16px"><div style="font-size:48px;margin-bottom:8px">🚀</div>';
    html += '<div style="font-size:20px;font-weight:700;color:var(--ink)">¡Tu plan está listo, ' + sanitize(ONBOARDING.data.name || '') + '!</div></div>';

    html += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px">';
    html += '<div style="text-align:center;background:var(--s2);border-radius:var(--rs);padding:12px"><div style="font-size:10px;color:var(--ink3);text-transform:uppercase;letter-spacing:.05em">Peso</div><div style="font-size:22px;font-weight:700;font-family:JetBrains Mono,monospace;color:var(--ink)">' + w + ' ' + u + '</div></div>';
    html += '<div style="text-align:center;background:var(--s2);border-radius:var(--rs);padding:12px"><div style="font-size:10px;color:var(--ink3);text-transform:uppercase;letter-spacing:.05em">Pull-ups</div><div style="font-size:22px;font-weight:700;font-family:JetBrains Mono,monospace;color:var(--blue)">' + r + '</div></div>';
    html += '</div>';

    html += '<div style="background:var(--bbg);border:1px solid var(--bborder);border-radius:var(--rs);padding:10px 14px;margin-bottom:10px;font-size:13px;color:var(--ink2)"><strong style="color:var(--ink)">Banda:</strong> ' + band.label + '</div>';
    html += '<div style="background:var(--gbg);border:1px solid var(--gborder);border-radius:var(--rs);padding:10px 14px;margin-bottom:14px;font-size:13px;color:var(--ink2)"><strong style="color:var(--ink)">Metas:</strong> Sem 4: ' + goals.week4 + ' · Sem 7: ' + goals.week7 + ' · Sem 10: ' + goals.week10 + '+</div>';

    // Optional PIN
    html += '<div style="background:var(--s2);border:1px solid var(--border);border-radius:var(--rs);padding:14px;margin-bottom:10px">';
    html += '<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px"><span style="font-size:18px">🔒</span><div><div style="font-size:13px;font-weight:600;color:var(--ink)">PIN de protección</div><div style="font-size:11px;color:var(--ink3)">Opcional — protege este perfil con 4 dígitos</div></div></div>';
    html += '<input type="password" id="ob-pin" class="sync-input" placeholder="4 dígitos" maxlength="4" inputmode="numeric" pattern="[0-9]*" style="text-align:center;font-size:18px;max-width:160px" class="pin-input-clean">';
    html += '</div>';
  }
  ...
  if (step < totalSteps) {
    html += '<button type="button" class="btn btn-blue" onclick="obNext()" style="flex:1;justify-content:center">Siguiente →</button>';
  } else {
  ...
  // Post-render: build emoji/color grids for step 2 (needs DOM)
  if (step === 2) {
    var emojiGrid = document.getElementById('ob-emoji-grid');
    if (emojiGrid) {
      PROFILE_EMOJIS.forEach(function(em) {
        ...
        btn.onclick = function() {
          selectedEmoji = em;
          emojiGrid.querySelectorAll('.profile-emoji-btn').forEach(function(b) { b.classList.toggle('selected', b.textContent === em); });
        };
        emojiGrid.appendChild(btn);
      });
    }
    var colorGrid = document.getElementById('ob-color-grid');
    if (colorGrid) {
      PROFILE_COLORS.forEach(function(c) {
        ...
        div.onclick = function() {
          selectedColor = c;
          colorGrid.querySelectorAll('.profile-color-btn').forEach(function(b) { b.classList.toggle('selected', b.style.background === c); });
          // Live preview theme color
          var theme = COLOR_THEMES.find(function(t) { return t.primary === c; });
          if (theme) applyColorTheme(theme.key);
        };
  ...
  // Auto-focus
  setTimeout(function() {
    var focusMap = { 1:'ob-name', 3:'ob-weight', 4:'ob-reps' };
    var inp = document.getElementById(focusMap[step]);
    if (inp) inp.focus();
  }, 200);
}
...
function obNext() {
  if (ONBOARDING.step === 1) {
    ...
    if (!name || name.length < 1) { showToast('Ingresa tu nombre', '⚠️'); return; }
    // Validate no duplicate names
    var profiles = getProfiles();
    var profileId = ONBOARDING.data.profileId;
    var nameLower = name.toLowerCase();
    var duplicate = profiles.find(function(p) { return p.id !== profileId && p.name.toLowerCase() === nameLower; });
    if (duplicate) { showToast('Ya existe un perfil con ese nombre', '⚠️'); return; }
    ONBOARDING.data.name = name;
  } else if (ONBOARDING.step === 2) {
    // Emoji + Color — save selections
    ONBOARDING.data.emoji = selectedEmoji;
    ONBOARDING.data.color = selectedColor;
...
async function obFinish() {
  ...
  // Optional PIN
  var pinVal = document.getElementById('ob-pin') ? document.getElementById('ob-pin').value.trim() : '';
  var pinHash = null;
  if (pinVal && pinVal.length === 4 && /^\d{4}$/.test(pinVal)) {
    pinHash = await hashPin(pinVal);
  } else if (pinVal && pinVal.length > 0) {
    showToast('El PIN debe ser 4 dígitos o dejarlo vacío', '⚠️');
    return;
  }

  // Resolve theme key from color
  var chosenColor = d.color || selectedColor || '#2563EB';
  var themeMatch = COLOR_THEMES.find(function(t) { return t.primary === chosenColor; });
  var themeKey = themeMatch ? themeMatch.key : 'blue';
...
async function hashPin(pin) {
  ...
  var data = new TextEncoder().encode('ig_salt_' + pin);
...
function renderPinSettingsUI() {
  ...
  if (hasPin) {
    ...
    html += '<input type="password" class="sync-input" id="pin-current-input" placeholder="••••" maxlength="4" inputmode="numeric" pattern="[0-9]*" style="max-width:120px;text-align:center;font-size:20px" class="pin-input-clean">';
    ...
    html += '<input type="password" class="sync-input" id="pin-change-new" placeholder="••••" maxlength="4" inputmode="numeric" pattern="[0-9]*" style="max-width:120px;text-align:center;font-size:20px" class="pin-input-clean">';
    ...
    html += '<input type="password" class="sync-input" id="pin-new-input" placeholder="••••" maxlength="4" inputmode="numeric" pattern="[0-9]*" style="max-width:120px;text-align:center;font-size:20px" class="pin-input-clean">';
    html += '<label style="font-size:12px;font-weight:500;color:var(--ink2)">Confirmar PIN</label>';
    html += '<input type="password" class="sync-input" id="pin-confirm-input" placeholder="••••" maxlength="4" inputmode="numeric" pattern="[0-9]*" style="max-width:120px;text-align:center;font-size:20px" class="pin-input-clean">';
...
// =====================================================================
// DEVICE ID + PROFILE LIMIT (max 2 per device)
// =====================================================================
var DEVICE_ID_KEY = 'ig_device_id';
var MAX_PROFILES_PER_DEVICE = 2;

function getDeviceId() {
  var id = LS.get(DEVICE_ID_KEY);
  if (!id) {
    ...
    LS.set(DEVICE_ID_KEY, id);
  }
  return id;
}

function getDeviceProfileCount() {
  // Count profiles created on THIS device
  var profiles = getProfiles();
  var deviceId = getDeviceId();
  return profiles.filter(function(p) {
    return p.deviceId === deviceId || !p.deviceId; // legacy profiles count too
  }).length;
}

function canCreateProfile() {
  var count = getDeviceProfileCount();
  if (count >= MAX_PROFILES_PER_DEVICE) {
    showModal({
      title: 'Limite de perfiles',
      message: 'Solo se permiten ' + MAX_PROFILES_PER_DEVICE + ' perfiles por dispositivo. Elimina un perfil existente para crear uno nuevo.',
      icon: '👥',
      type: 'alert',
      confirmText: 'Entendido'
    });
...
// ===================================================================
var PROFILE_EMOJIS = ['😊','😎','🤩','😤','🥶','🧔','👩','👦','👧','🧑','👨','👩‍🦰','🧒','🤠','🦸','🦹'];
var PROFILE_COLORS = COLOR_THEMES.map(function(t) { return t.primary; });
var CURRENT_PROFILE_KEY = 'ig_current_profile';
var PROFILES_INDEX_KEY  = 'ig_profiles';
var selectedEmoji = '💪';
var selectedColor = '#2563EB';

// Profile data structure
// localStorage keys:
//   ig_profiles        → JSON array of {id, name, emoji, color, createdAt}
//   ig_current_profile → current profile id
//   barpull_{id}       → DB for each profile
...
function getDBKey(profileId) {
  return profileId === 'default' ? DB_KEY_PREFIX : 'barpull_' + profileId;
}
...
function renderProfileList() {
  ...
  if (countLabel) {
    var used = getDeviceProfileCount();
    countLabel.textContent = used + '/' + MAX_PROFILES_PER_DEVICE + ' perfiles';
    countLabel.style.color = used >= MAX_PROFILES_PER_DEVICE ? 'var(--red)' : 'var(--ink3)';
  }
}
...
function switchProfile(profileId) {
  ...
  try {
    ...
    if (saved) {
      ...
      // New profile — start fresh from schema defaults
      DB = JSON.parse(JSON.stringify(DB_DEFAULTS));
    }
    // Ensure all fields (same logic as loadDB)
    if (!Array.isArray(DB.pullupLog)) DB.pullupLog = [];
    if (!Array.isArray(DB.weights)) DB.weights = [];
    if (!Array.isArray(DB.sessions)) DB.sessions = [];
    if (!DB.records || typeof DB.records !== 'object') DB.records = {};
  ...
  applyColorTheme(DB.settings.colorTheme || 'blue');
  // Update UI
  renderProfileAvatar();
  renderLog();
  renderTotalStats();
  ...
  renderThemePicker();
  document.getElementById('profile-dropdown').classList.add('hidden');
  // Reset HOY state if not mid-training
  if (!HOY_STATE.seriesDone) resetHoy();
}
...
async function showCreateProfile() {
  ...
  document.getElementById('profile-dropdown').classList.add('hidden');
  // Confirmation modal
  var ok = await modalConfirm('Nuevo perfil', '¿Crear un nuevo perfil? Te guiaremos paso a paso para configurarlo.', '👤');
  if (!ok) return;
  // Create bare profile with defaults
  var id = 'p_' + Date.now();
  var profiles = getProfiles();
  var randomEmoji = PROFILE_EMOJIS[Math.floor(Math.random() * PROFILE_EMOJIS.length)];
  var newProfile = {id:id, name:'Nuevo', emoji:randomEmoji, color:'#2563EB', createdAt:localDateStr(), deviceId:getDeviceId()};
  profiles.push(newProfile);
  saveProfiles(profiles);
  // Reset wizard selections
  selectedEmoji = randomEmoji;
  selectedColor = '#2563EB';
  // Switch to new profile (loads empty DB)
  window._skipGuestCheck = true;
  switchProfile(id);
  // Launch onboarding wizard
  setTimeout(function() {
    window._skipGuestCheck = false;
    startOnboarding(id);
  }, 300);
}

// Legacy — kept for HTML panel compat but no longer primary flow
function createProfile() { showCreateProfile(); }
...
function initOfflineDetection() {
  function updateOnlineStatus() {
    ...
    if (isOffline) {
      if (isLoggedIn()) {
        showToast('Sin conexion — los cambios se sincronizaran al reconectar', '📡', 4000);
        updateSyncIndicator('offline');
      } else {
        showToast('Sin conexion — los datos se guardan localmente', '⚠️', 4000);
      }
    } else {
      // Back online — flush sync queue
      if (isLoggedIn()) {
        updateSyncIndicator('synced');
        flushSyncQueue();
      }
...
// ===================================================================
var SESSION_DRAFT_KEY = 'ig_session_draft';
...
function renderQuickWeight() {
  var el = document.getElementById('quick-weight-widget');
  if (!el) return;
  var unit = getWeightUnit();
  var todayStr = localDateStr();
  var todayEntry = DB.weights.find(function(w) { return w.date === todayStr; });
  ...
  if (todayEntry) {
    // SAVED STATE — show weight + link to peso tab
    el.innerHTML = '<div style="flex:1">' +
      '<div style="font-size:11px;color:var(--ink3);text-transform:uppercase;letter-spacing:.06em;margin-bottom:2px">Peso de hoy</div>' +
      '<div style="font-size:15px;font-weight:700;color:var(--ink)">' + todayEntry.weight + ' ' + unit + ' <span style="font-size:11px;font-weight:400;color:var(--green)">✓</span></div>' +
      '</div>' +
      '<button type="button" class="btn btn-ghost btn-sm" onclick="goToPesoTab()">📊 Ver peso</button>';
  } else {
    // INPUT STATE — show input + save button
    var w = getProfileWeight();
    el.innerHTML = '<div style="flex:1">' +
      '<div style="font-size:11px;color:var(--ink3);text-transform:uppercase;letter-spacing:.06em;margin-bottom:2px">Peso de hoy</div>' +
      '<div style="font-size:13px;font-weight:600;color:var(--ink)">Sin registrar</div>' +
      '</div>' +
      '<input type="number" class="quick-weight-input" id="quick-weight-inp" aria-label="Peso de hoy" placeholder="' + (w ? Math.round(w) : '') + '" step="0.1" min="30" max="500" onkeydown="if(event.key===\'Enter\')quickWeightSave()">' +
      '<span style="font-size:13px;color:var(--ink3)">' + unit + '</span>' +
      '<button type="button" class="btn btn-blue btn-sm" aria-label="Guardar peso" onclick="quickWeightSave()">Guardar</button>';
  }
}

function goToPesoTab() {
  var btn = document.querySelectorAll('.nb')[2]; // Progreso button
  goMain('progreso', btn);
  setTimeout(function() {
    NAV_MAP.progreso._lastSub = 'peso';
    goSub('peso', document.querySelector('.snb:nth-child(2)'));
  }, 50);
}
...
if ('serviceWorker' in navigator) {
  var swVersion = APP_VERSION.replace(/\./g,'');
  var swCode = [
    "var CACHE='barpull-v'+'" + swVersion + "';",
    "var FILES=['./','./index.html','./styles.css','./app.js'];",
    ...
    "    return Promise.all(keys.filter(function(k){return k.startsWith('barpull-')&&k!==CACHE;}).map(function(k){return caches.delete(k);}));",
    "  }));",
    "  self.clients.claim();",
    "});",
    // Fetch: cache-first for app files, network-first for API/CDN
    "self.addEventListener('fetch',function(e){",
    "  var url=e.request.url;",
    "  if(url.includes('supabase.co')||url.includes('cdn.jsdelivr.net')||url.includes('fonts.g')){",
