...
function finishSession() {
  if (confirm('¿Terminar el entrenamiento ahora?')) {
    completeAllExercises();
  }
}
...
function estimateCalories(durationSec, exercises) {
  ...
  var weightLb = 208; // default
  if (DB.weights && DB.weights.length > 0) {
    weightLb = DB.weights[0].weight; // most recent
  }
  var weightKg = weightLb * 0.4536;
...
function confirmReset() {
  if (confirm('¿Borrar TODOS los datos de este perfil? Esta acción no se puede deshacer.')) {
...
function confirmFullReset() {
  if (!confirm('⚠️ ¿Borrar ABSOLUTAMENTE TODO?\n\nEsto borra todos los perfiles, todos los datos, configuración de GitHub — todo vuelve a cero.\n\nEsta acción NO se puede deshacer.')) return;
...
function removePin() {
  if (!confirm('¿Quitar la protección PIN de este perfil?')) return;
  var profiles = getProfiles();
  var currentId = getCurrentProfileId();
  var profile = profiles.find(function(p) { return p.id === currentId; });
  if (profile) {
