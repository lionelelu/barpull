...
function loadDB() {
  try {
    ...
    var dbKey = profileId === 'default' ? DB_KEY_PREFIX : 'barpull_v2_' + profileId;
...
async function confirmFullReset() {
  ...
  for (var i = 0; i < localStorage.length; i++) {
    ...
    if (key && (key.startsWith('barpull') || key.startsWith('irongrip') || key.startsWith('ig_') || key === 'bp_migrated')) {
      keys.push(key);
    }
  }
...
window.addEventListener('DOMContentLoaded', function() {
  ...
  // Migrate legacy irongrip_v2* keys to barpull_v2*
  (function() {
    try {
      var migrated = LS.get('bp_migrated');
      if (migrated) return;
      for (var i = 0; i < localStorage.length; i++) {
        var key = localStorage.key(i);
        if (key && key.startsWith('irongrip_v2')) {
          var newKey = key.replace('irongrip_v2', 'barpull_v2');
          if (!localStorage.getItem(newKey)) {
            localStorage.setItem(newKey, localStorage.getItem(key));
          }
        }
      }
      LS.set('bp_migrated', '1');
    } catch(e) { console.log('Migration skipped', e); }
  })();
  loadDB();
...
function toggleGHCard() {
  var body = document.getElementById('gh-card-body');
  if (body) body.style.display = body.style.display === 'none' ? 'block' : 'none';
}

function ghInit() {
  GH.username = DB.settings.ghUsername || null;
  GH.repo     = DB.settings.ghRepo     || 'barpull';
  // Load token from separate storage (not stored in main DB)
  try {
    var profileId = LS.get(CURRENT_PROFILE_KEY) || 'default';
    var tokenKey = profileId === 'default' ? 'ig_t' : 'ig_t_' + profileId;
    ...
    if (DB.settings.ghToken) {
      var _tKey = (LS.get(CURRENT_PROFILE_KEY)||'default') === 'default' ? 'ig_t' : 'ig_t_'+(LS.get(CURRENT_PROFILE_KEY)||'default');
...
function compressData() {
  ...
  if (data.settings) {
    ...
    delete data.settings.ghFileSha;
    delete data.settings.ghLastSync;
    delete data.settings.ghUsername;
    delete data.settings.ghRepo;
  }
...
async function importSyncCode() {
  ...
  // Handle BPULL: prefix (also accept legacy IGRIP: prefix)
  if (code.startsWith('BPULL:')) code = code.substring(6);
  else if (code.startsWith('IGRIP:')) code = code.substring(6);
  ...
  try {
    ...
    if (ok) {
      ...
      var localSettings = {
        ...
        ghUsername: DB.settings.ghUsername,
        ghRepo: DB.settings.ghRepo,
...
// OFFLINE SYNC QUEUE
// =====================================================================
var SYNC_QUEUE_KEY = 'ig_sync_queue';

function queueSync() {
  // Mark that we have pending changes to sync
  LS.set(SYNC_QUEUE_KEY, JSON.stringify({
    pending: true,
    queuedAt: Date.now(),
...
function clearSyncQueue() {
  LS.remove(SYNC_QUEUE_KEY);
}

function hasPendingSync() {
  try {
    var q = LS.getJSON(SYNC_QUEUE_KEY, null);
    return q && q.pending;
...
async function processSyncQueue() {
  ...
  // Try GitHub sync if connected
  if (GH && GH.token && GH.username && !GH._saving) {
    await ghSave();
    clearSyncQueue();
    showToast('Cambios sincronizados con GitHub', '🔄', 2500);
  } else {
    clearSyncQueue();
  }
}
...
function ghSaveConfig() {
  ...
  var r = document.getElementById('gh-repo-input').value.trim() || 'barpull';
  ...
  var _tKey = _profId === 'default' ? 'ig_t' : 'ig_t_' + _profId;
...
function ghDisconnect() {
  ...
  var _tkKey = _pId === 'default' ? 'ig_t' : 'ig_t_' + _pId;
  LS.remove(_tkKey);
  saveDBLocal();
  ghUpdateUI();
}
...
async function ghSave() {
  ...
  try {
    ...
    var body = {
      message: 'Sync BarPull ' + new Date().toLocaleDateString('es'),
      content: encoded,
      committer: { name: 'BarPull', email: 'app@barpull.local' }
    };
...
//   barpull_v2_{id}    → DB for each profile
//   ig_t_{id}          → GitHub token for each profile
...
function getDBKey(profileId) {
  return profileId === 'default' ? DB_KEY_PREFIX : 'barpull_v2_' + profileId;
}

function getTokenKey(profileId) {
  return profileId === 'default' ? 'ig_t' : 'ig_t_' + profileId;
}
...
if ('serviceWorker' in navigator) {
  ...
  var swCode = [
    ...
    "    return Promise.all(keys.filter(function(k){return (k.startsWith('barpull-')||k.startsWith('irongrip-'))&&k!==CACHE;}).map(function(k){return caches.delete(k);}));",
    "  }));",
    "  self.clients.claim();",
    "});",
    ...
    "  if(url.includes('api.github.com')||url.includes('fonts.g')){",
