...
function toggleInlineExercise(el, exName) {
  ...
  var existing = el.parentNode.querySelector('.inline-ex-detail');
  if (existing) {
    existing.remove();
    return;
  }
  // Close any other open details
  document.querySelectorAll('.inline-ex-detail').forEach(function(d) { d.remove(); });
  // Get exercise data
  var html = getInstructionsHTML(exName);
  if (!html || html.includes('pestaña')) {
    // Try aliases
    var aliases = {
  ...
  if (!html || html.includes('pestaña')) return;
  var detail = document.createElement('div');
  detail.className = 'inline-ex-detail';
