...
function loadDB() {
  try {
    ...
    var dbKey = profileId === 'default' ? DB_KEY_PREFIX : 'irongrip_v2_' + profileId;
...
function generateShareCard() {
  ...
  ctx.font = '10px JetBrains Mono, monospace';
  ctx.textAlign = 'right';
  ctx.fillText('irongrip · ' + localDateStr(), W - 20, H - 15);
  ctx.textAlign = 'left';
  return canvas;
}
...
function shareProgress() {
  ...
  canvas.toBlob(function(blob) {
    ...
    if (navigator.share && navigator.canShare) {
      var file = new File([blob], 'irongrip-progreso.png', { type: 'image/png' });
      var shareData = { files: [file], title: 'Mi progreso — IronGrip' };
...
function downloadShareImage(blob) {
  ...
  a.href = url;
  a.download = 'irongrip-progreso-' + localDateStr() + '.png';
  a.click();
...
function setupNotifications() {
  ...
  Notification.requestPermission().then(function(perm) {
    ...
    if (perm === 'granted') {
      ...
      new Notification('IronGrip', { body: '¡Notificaciones activadas! Te avisaremos cuando sea hora de entrenar.', icon: '' });
    } else {
...
async function confirmFullReset() {
  ...
  for (var i = 0; i < localStorage.length; i++) {
    ...
    if (key && (key.startsWith('irongrip') || key.startsWith('ig_'))) {
      keys.push(key);
    }
  }
...
function exportData() {
  ...
  var a = document.createElement('a');
  a.href = url;
  a.download = 'irongrip_datos_' + localDateStr() + '.json';
  a.click();
  URL.revokeObjectURL(url);
}
...
window.addEventListener('DOMContentLoaded', function() {
  ...
  (function() {
    var manifest = {"name":"IronGrip","short_name":"IronGrip","description":"Tu rutina de dominadas","start_url":"./","display":"standalone","background_color":"#F6F5F1","theme_color":"#2563EB","orientation":"portrait-primary"};
    try {
...
function ghInit() {
  ...
  GH.repo     = DB.settings.ghRepo     || 'irongrip';
...
function copySyncCode() {
  var data = compressData();
  var code = 'IGRIP:' + btoa(unescape(encodeURIComponent(data)));
...
function shareBackupFile() {
  var data = compressData();
  var blob = new Blob([data], { type: 'application/json' });
  var fileName = 'irongrip-backup-' + localDateStr() + '.json';
  
  if (navigator.share && navigator.canShare) {
    var file = new File([blob], fileName, { type: 'application/json' });
    var shareData = { files: [file], title: 'IronGrip - Backup' };
    if (navigator.canShare(shareData)) {
...
async function importSyncCode() {
  ...
  // Handle IGRIP: prefix
  if (code.startsWith('IGRIP:')) code = code.substring(6);
...
function ghSaveConfig() {
  ...
  var r = document.getElementById('gh-repo-input').value.trim() || 'irongrip';
...
async function ghSave() {
  ...
  try {
    ...
    var body = {
      message: 'Sync IronGrip ' + new Date().toLocaleDateString('es'),
      content: encoded,
      committer: { name: 'IronGrip', email: 'app@irongrip.local' }
    };
...
//   irongrip_v2_{id}    → DB for each profile
...
function getDBKey(profileId) {
  return profileId === 'default' ? DB_KEY_PREFIX : 'irongrip_v2_' + profileId;
}
...
if ('serviceWorker' in navigator) {
  ...
  var swCode = [
    "var CACHE='irongrip-v'+'" + swVersion + "';",
    ...
    "    return Promise.all(keys.filter(function(k){return k.startsWith('irongrip-')&&k!==CACHE;}).map(function(k){return caches.delete(k);}));",
    "  }));",
    "  self.clients.claim();",
    "});",
