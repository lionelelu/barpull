...
// ═══════════════════════════════════════════════════════════════════
var DB = {
  settings: { dark:false, voice:true, sound:true, vibration:true, wakelock:true, prepDelay:5 },
  pullupLog: [{ sem:'Sem 1', pu:4, banda:'roja', diff:null }],
  weights: [],
  sessions: [],
  records: {},
  totalTrainingTime: 0,
...
function loadDB() {
  try {
    ...
    if (saved) {
      ...
      // Ensure arrays exist
      if (!Array.isArray(DB.pullupLog)) DB.pullupLog = [{ sem:'Sem 1', pu:4, banda:'roja', diff:null }];
      if (!Array.isArray(DB.weights)) DB.weights = [];
      if (!Array.isArray(DB.sessions)) DB.sessions = [];
      if (!DB.records) DB.records = {};
      if (!DB.settings) DB.settings = { dark:false, voice:true, sound:true, vibration:true, wakelock:true, prepDelay:5 };
    }
...
function go(id, btn) {
  ...
  if (id === 'progresion') { renderProgresion(); }
...
function startTraining() {
  ...
  if (dayData.warmup && !HOY_STATE.warmupDone) {
    alert('¡Primero completa el calentamiento! Toca "Iniciar calentamiento".');
    return;
  }
...
function clickSerieBadge(exIdx, serieIdx) {
  // Allow clicking badges to jump to that serie
  HOY_STATE.exIdx = exIdx;
  HOY_STATE.serieIdx = serieIdx;
}
...
function resetHoy() {
  ...
  document.getElementById('hoy-training').style.display = 'none';
  document.getElementById('warmup-strip').style.display = 'none';
  document.getElementById('session-summary').classList.remove('show');
...
function confirmSkipWarmup() {
  if (confirm('¿Estás seguro? Saltarse el calentamiento aumenta el riesgo de lesiones en los hombros.')) {
    document.getElementById('warmup-mode').style.display = 'none';
    HOY_STATE.warmupDone = true;
    document.getElementById('hoy-start-panel').style.display = 'block';
    var startBtn = document.getElementById('start-train-btn');
...
function showMiniTab(idx, tab, btn) {
  document.querySelectorAll('.mini-content').forEach(c => c.classList.remove('on'));
  document.querySelectorAll('.mini-tab').forEach(b => b.classList.remove('on'));
  var el = document.getElementById('mini-' + tab + '-' + idx);
...
function saveLog() {
  ...
  if (!sem) { alert('Selecciona la semana'); return; }
  if (isNaN(pu) || pu < 1 || pu > 100) { alert('Ingresa un número válido de pull-ups'); return; }
  var prev = DB.pullupLog[DB.pullupLog.length-1].pu;
  DB.pullupLog.push({ sem, pu, banda, diff: pu-prev });
  saveDB();
  renderLog();
  checkAchievements();
  ['inp-sem','inp-pu','inp-banda'].forEach(id => { document.getElementById(id).value = ''; });
}

function renderLog() {
  renderWeekSummary();
  renderWeekComparison();
  var best = Math.max(...DB.pullupLog.map(e => e.pu));
  ...
  body.innerHTML = DB.pullupLog.map(function(e, i) {
    ...
    return '<tr><td style="font-family:JetBrains Mono,monospace;font-size:12px">'+semSafe+'</td>'
      ...
      +'<td>'+d+'</td>'
      +'<td style="font-size:12px">'+bandaSafe+'</td></tr>';
  }).join('');
}
...
function renderSessionHistory() {
  ...
  if (DB.sessions.length === 0) {
    renderEmptyState('session-history-list', '🏋️', 'Sin sesiones aún', 'Completa tu primer entrenamiento desde la pestaña Hoy y aparecerá aquí con todos los detalles.', '¡Empezar ahora!', "go('hoy',document.querySelector('.nb'))");
    return;
  }
  ...
  DB.sessions.slice(0,10).forEach(function(s) {
    ...
    html += '<div style="font-size:12px;color:var(--ink3)">'+s.date+' · '+s.seriesDone+' series · '+m+'min '+sec+'s</div>';
...
function drawProgressChart() {
  ...
  // Pull-up data
  var puData = DB.pullupLog.map(e => e.pu);
...
function renderPeso() {
  ...
  if (inicioEl) inicioEl.textContent = DB.weights.length > 0 ? DB.weights[DB.weights.length-1].weight : '208';
  // Change
...
function savePeso() {
  ...
  if (!rawVal || isNaN(val) || val < 80 || val > 500) {
    alert('Ingresa un peso válido (entre 80 y 500 lb)');
    return;
  }
...
function renderPesoLog() {
  ...
  DB.weights.slice(0,30).forEach(function(w, i) {
    ...
    html += '<div class="weight-log-row"><div class="weight-log-date">'+w.date+'</div><div style="display:flex;align-items:center;gap:8px"><div class="weight-log-val">'+w.weight+' lb</div>'+diffHtml+'</div></div>';
  });
  el.innerHTML = html;
}
...
function getTotalTrainingTime() {
  // Sum duration of all sessions (stored in seconds)
  var total = DB.totalTrainingTime || 0;
  // Also sum from sessions array if available
  DB.sessions.forEach(function(s) {
    if (s.duration && !isNaN(s.duration)) total += s.duration;
  });
...
function confirmReset() {
  if (confirm('¿Borrar TODOS los datos? Esta acción no se puede deshacer.')) {
    localStorage.removeItem(DB_KEY_PREFIX);
    location.reload();
  }
}
...
window.addEventListener('DOMContentLoaded', function() {
  ...
  var todayStr = localDateStr();
  DB.sessions.filter(s => s.date === todayStr).forEach(s => {
...
async function ghLoad() {
  ...
  try {
    ...
    if (confirm('¿Reemplazar datos locales con los de GitHub? Esto sobreescribirá tu progreso local.')) {
...
function createProfile() {
  ...
  var profiles = getProfiles();
  if (profiles.length >= 10) { alert('Máximo 10 perfiles'); return; }
  var id = 'p_' + Date.now();
...
function deleteProfile(event, profileId) {
  ...
  if (!profile) return;
  if (!confirm('¿Eliminar el perfil "' + profile.name + '"? Se borrarán todos sus datos.')) return;
  // Remove data
...
var TOKEN_PREFIX  = 'bpro_t';
var MAX_PROFILES  = 10;
var SYNC_DEBOUNCE = 8000; // ms

// ═══════════════════════════════════════════════════════════════════
// SIMPLE EVENT BUS (for future inter-module communication)
// ═══════════════════════════════════════════════════════════════════
var Events = {
  _listeners: {},
...
function generateWeeklyChallenge() {
  ...
  var last=DB.pullupLog.length>0?DB.pullupLog[DB.pullupLog.length-1].pu:4;
  var pool=[
...
function initOfflineDetection() {
  function updateOnlineStatus() {
    ...
    Events.emit('network:change', { online: !isOffline });
    if (isOffline) {
...
function showToast(message, icon, duration) {
  ...
  toast.innerHTML = '<span style="font-size:18px">' + sanitize(String(icon)) + '</span><span>' + sanitize(String(message)) + '</span>';
...
function resumeSession() {
  ...
  var hoyBtn = document.querySelector('.nb');
  if (hoyBtn) go('hoy', hoyBtn);
...
function initSwipeNav() {
  var tabs = ['hoy','programa','ejercicios','bandas','progresion','peso','tracker','logros','ajustes'];
...
function quickWeightSave() {
  ...
  Events.emit('weight:saved', { weight: val, date: todayStr });
...
// ═══════════════════════════════════════════════════════════════════
// FUTURE-PROOF: PLUGIN/EXTENSION REGISTRY
// (allows adding features without touching core code)
// ═══════════════════════════════════════════════════════════════════
var App = {
  version: APP_VERSION,
  events: Events,
  ...
  sanitize: sanitize,
  toast: showToast,
  // Register a new tab (for future expansion)
  registerTab: function(config) {
    // config: { id, label, icon, render }
    console.log('Tab registered:', config.id);
    Events.emit('tab:registered', config);
  },
  // Register a new achievement type
  registerAchievement: function(achievement) {
    BADGES_DEF.push(achievement);
    Events.emit('achievement:registered', achievement);
  }
};
