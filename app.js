...
function checkBackupReminder() {
  var lastBackup = LS.get('bpro_last_backup', '0');
  ...
  if (sessions >= 3 && daysSince >= 7) {
    setTimeout(function() {
      showModal({
        ...
        if (ok) {
          shareBackupFile();
          LS.set('bpro_last_backup', String(Date.now()));
        }
      });
    }, 5000);
  }
...
function copySyncCode() {
  var data = compressData();
  var code = 'BPRO:' + btoa(unescape(encodeURIComponent(data)));
...
function shareBackupFile() {
  var data = compressData();
  var blob = new Blob([data], { type: 'application/json' });
  var fileName = 'barra-pro-backup-' + localDateStr() + '.json';
  
  if (navigator.share && navigator.canShare) {
    var file = new File([blob], fileName, { type: 'application/json' });
    var shareData = { files: [file], title: 'Barra Pro - Backup' };
    if (navigator.canShare(shareData)) {
...
async function importSyncCode() {
  ...
  // Handle BPRO: prefix
  if (code.startsWith('BPRO:')) code = code.substring(5);
...
// OFFLINE SYNC QUEUE
// =====================================================================
var SYNC_QUEUE_KEY = 'bpro_sync_queue';
...
function renderOnboardingStep() {
  ...
  // Auto-focus
  setTimeout(function() {
    var focusMap = { 1:'ob-name', 2:'ob-weight', 3:'ob-reps' };
...
// =====================================================================
var DEVICE_ID_KEY = 'bpro_device_id';
...
function selectColor(color) {
  ...
  // Preview theme color immediately
  var theme = COLOR_THEMES.find(function(t) { return t.primary === color; });
  if (theme) applyColorTheme(theme.key);
}

function createProfile() {
  // Check device profile limit
  ...
  var id = 'p_' + Date.now();
  var newProfile = {id:id, name:name, emoji:selectedEmoji, color:selectedColor, createdAt:localDateStr(), deviceId:getDeviceId()};
  profiles.push(newProfile);
  saveProfiles(profiles);
  // Switch to new profile (this loads empty DB)
  window._skipGuestCheck = true;
  switchProfile(id);
  // Trigger onboarding wizard (only once)
  setTimeout(function() {
...
if ('serviceWorker' in navigator) {
  ...
  var swCode = [
    "var CACHE='barra-pro-v'+'" + swVersion + "';",
    ...
    "    return Promise.all(keys.filter(function(k){return k.startsWith('barra-pro-')&&k!==CACHE;}).map(function(k){return caches.delete(k);}));",
    "  }));",
    "  self.clients.claim();",
    "});",
